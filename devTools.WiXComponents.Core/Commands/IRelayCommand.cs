...
using devTools.WiXComponents.Core.Extensions;
...
namespace devTools.WiXComponents.Core.Commands
{
	...
	public static class IRelayCommandExtension
	{
			...
			where T : IRelayCommand
		{
			...
			obj.PropertyChanged += (_, e) =>
			{
				if (propertyNames.All(propertyName => propertyName != e.PropertyName)) return;
				if (dispatcher != null) dispatcher.Run(thisValue.RaiseCanExecuteChanged);
				else thisValue.RaiseCanExecuteChanged();
			};
			return thisValue;
			...
			where T : IRelayCommand
		{
			...
			obj.PropertyChanged += (_, e) =>
			{
				if (!propertyNames.Contains(e.PropertyName)) return;
				if (dispatcher != null) dispatcher.Run(thisValue.RaiseCanExecuteChanged);
				else thisValue.RaiseCanExecuteChanged();
			};
			return thisValue;
